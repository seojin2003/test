...
<%@ include file="/WEB-INF/jsp/common/header.jsp" %>
	...
	<section class="mt-8">
		<div class="container mx-auto">
			...
			<c:choose>
				<c:when test="${empty sessionScope.loginedMember or sessionScope.loginedMember.id == 0}">
					<!-- 로그인하지 않은 경우 글쓰기 버튼 숨김 -->
				</c:when>
				<c:otherwise>
					<c:choose>
						<c:when test="${sessionScope.loginedMember.authLevel == 0 }">
							<div class="px-6 pt-6">
								<div class="text-right"><a class="btn btn-neutral btn-outline btn-xs" href="write?boardId=${board.getId()}">글쓰기</a></div>
							</div>
						</c:when>
						<c:otherwise>
